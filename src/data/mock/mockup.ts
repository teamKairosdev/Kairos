...
const EXAMPLE_METRIC_LABEL = '예시 수치';
  ...
  for (let i = 1; i <= 50; i++) {
    ...
    const careers: MockCareer[] = [
      {
        ...
        role: `시니어 ${job.title}`,
        period: '2023.03 ~ 현재',
        description: `${job.tech} 기반 주요 프로덕트의 아키텍처 개선 및 성과 주도. ${EXAMPLE_METRIC_LABEL}: MAU 500만+ 및 비즈니스 지표 15% 이상 성장.`
      },
      {
        id: `mock-career-${i}-2`,
        userId: user.id,
        ...
        description: `신규 피처 개발 및 성능 최적화 참여. ${EXAMPLE_METRIC_LABEL}: 로딩 성능 30% 단축 및 컴포넌트 공통화 작업으로 팀 생산성 향상.`
      }
    ];

    const originalContent = `# ${name} - ${job.title} 이력서\n\n## 소개\n${job.title}로써 여러 대형 플랫폼 서비스 개발 및 운영을 성공적으로 수행하였습니다.\n\n## 기술 스택\n${job.tech}\n\n## 경력 사항\n### ${co1} (2023.03 ~ 현재)\n- ${job.title} 팀 내 개발 업무 담당\n- 프로젝트 최적화 및 쿼리 튜닝 진행\n\n### ${co2} (2020.01 ~ 2023.02)\n- 신규 서비스 런칭 참여\n- 프론트/백 백오피스 개발`;

    const improvedContent = `# ${name} | Senior ${job.field} Engineer\n\n## Executive Summary\n대규모 트래픽 및 비즈니스 변화가 빠른 환경에서 ${job.tech}을 바탕으로 고품질 소프트웨어를 배포한 개발자. 성능 최적화와 DX 개선에 강점이 있음.\n\n## 주요 성과\n- **${co1}**: ${EXAMPLE_METRIC_LABEL}: API 응답 지연 시간 25% 단축 및 코드 결합도 제거를 통한 신규 피처 배포 속도 2배 개선\n- **${co2}**: ${EXAMPLE_METRIC_LABEL}: 공통 모듈 구축을 통한 코드 재사용률 45% 확보 및 에러 모니터링 구축을 통한 앱 크래시율 0.2% 달성`;
    ...
    const interviewChats: Record<string, MockMessage[]> = {
      [`mock-interview-${i}-1`]: [
        ...
        { role: 'user', content: `안녕하세요. 저는 ${co1}에서 ${job.tech}을 활용하여 성능 최적화를 주도했습니다. 특히 ${EXAMPLE_METRIC_LABEL}로 API 응답 속도를 기존 대비 25% 단축한 경험을 소개하겠습니다.` },
        { role: 'assistant', content: `${EXAMPLE_METRIC_LABEL}로 제시한 API 성능 25% 단축의 구체적인 기법은 무엇이었나요? 쿼리 튜닝, 캐싱, 혹은 아키텍처상의 레이어 분리였는지 설명해주십시오.` }
      ]
    };
...
export function getSimulatedLLMResponse(message: string, context?: string): { responseText: string; suggestedContent?: string } {
  ...
  if (lowercaseMsg.includes('이력서') || lowercaseMsg.includes('자기소개서') || lowercaseMsg.includes('경력')) {
    return {
      ...
      suggestedContent: (context || '') + `\n\n- [AI 수정제안] ${EXAMPLE_METRIC_LABEL}: 대고객 핵심 결제 모듈 리팩토링 및 쿼리 캐싱 패턴 도입으로 전체 트랜잭션 에러율 0.05% 이하 제어 및 응답속도 LCP 지표 45% 단축.`
    };
  }
  ...
  if (lowercaseMsg.includes('스택') || lowercaseMsg.includes('기술')) {
    return {
      ...
      suggestedContent: (context || '') + `\n\n- [AI 수정제안] Next.js 16 App Router 및 Server Component 패턴을 점진적 채택하여 데이터 페칭 아키텍처 단순화 및 ${EXAMPLE_METRIC_LABEL}: 초기 TBT(Total Blocking Time) 300ms 감소.`
    };
  }

  return {
    responseText: `질문하신 내용에 대한 보강 추천 피드백을 전달해 드립니다. 프로젝트의 맥락과 구현 과정을 수치로 증명하는 내용을 추가하면 서류 평가와 면접에서 더욱 유리한 피드백을 받을 수 있습니다.`,
    suggestedContent: (context || '') + `\n\n- [AI 수정제안] ${EXAMPLE_METRIC_LABEL}: 클라우드 인프라 자원의 프로비저닝 자동화(IaC, Terraform) 기법을 설계하여 아키텍처 일관성 유지 및 리소스 유휴 비용 20% 최적화.`
  };
}
