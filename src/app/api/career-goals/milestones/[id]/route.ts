...
async function updateMilestone(req: NextRequest, id: string, userId: string) {
  ...
  if (Object.prototype.hasOwnProperty.call(body, 'status') && !['pending', 'completed', 'skipped'].includes(String(body.status).toLowerCase())) {
    return badRequest('마일스톤 상태가 올바르지 않습니다.');
  }

  const db = getDb();
  if (!db) {
    const existing = getFallbackMilestone(userId, id);
    ...
    const updated = updateFallbackMilestone(userId, id, patch);
    return updated ? demoJson(updated) : notFound('마일스톤을 찾을 수 없거나 권한이 없습니다.');
  }

  const [existing] = await db
    .select()
    .from(careerMilestones)
    .where(and(eq(careerMilestones.id, id), eq(careerMilestones.userId, userId)));
  if (!existing) return notFound('마일스톤을 찾을 수 없거나 권한이 없습니다.');
...
export async function GET(
  ...
  try {
    ...
    if (!id) return badRequest('마일스톤 ID가 필요합니다.');
    const db = getDb();
    if (!db) {
      const item = getFallbackMilestone(session.userId, id);
      return item ? demoJson(item) : notFound('마일스톤을 찾을 수 없거나 권한이 없습니다.');
    }
    const [item] = await db
      .select()
      .from(careerMilestones)
      .where(and(eq(careerMilestones.id, id), eq(careerMilestones.userId, session.userId)));
...
export async function PUT(
  ...
  try {
    ...
    const { id } = await params;
    if (!id) return badRequest('마일스톤 ID가 필요합니다.');
    return updateMilestone(req, id, session.userId);
  } catch (err: unknown) {
...
export async function DELETE(
  ...
  try {
    ...
    const db = getDb();
    if (!db) {
      return deleteFallbackMilestone(session.userId, id)
