...
## 세션 10: 문제 정의·자료조사 섹션 심층 확장 (2026-08-03)

### 1. 리서치 (병렬 서브에이전트 8개 — 통계/기사/보고서 원문 URL 수집)
- [x] R1: '쉬었음' 청년·재취업 번아웃·구직단념 심층 (기사 링크) — 14 URL
- [x] R2: 통계청 청년 부가조사·경제활동인구조사 원문 통계 (URL) — 30+ URL
- [x] R3: AI 자가소서·면접·탐지 시장 (무하유·사람인·진학사·고용노동부, URL) — 33 URL
- [x] R4: ATS 최적화·이력서 데이터 (Jobscan 등, URL) — 17 URL
- [x] R5: 국내 AI 채용·커리어 서비스 + 글로벌 벤치 (원티드·랠릿·Teal 등, URL) — 35+ URL
- [x] R6: 커리어 코칭·진로심리 기법 (만다라트·RIASEC, URL) — 32 URL
- [x] R7: 이직률·기술시장 트렌드 (워크넷·고용노동부, URL) — 21 URL
- [x] R8: 커리어 데이터 관리 트렌드 (랠릿 MY 로그·나머지·Notion, URL) — 31 URL

### 2. 구성/검증
- [ ] 구성 서브에이전트 — 2회 연속 빈 응답(파일 미변경 확인) → 참조 파일 분리 + 소규모 에이전트로 재시도 필요
- [ ] 검증 서브에이전트 — 형식 규칙·원문 무손실·링크 존재 검증
- [ ] 커밋 (push 안 함)

### 2. 구성/검증
- [ ] 구성 서브에이전트 — `AI서비스톤_통합계획서.md` 섹션 3 재구조화(3-1~3-12) + 기존 원문·통계 라인 보존 + 링크 삽입
- [ ] 검증 서브에이전트 — 형식 규칙·원문 무손실·링크 존재 검증
- [ ] 커밋 (push 안 함)
