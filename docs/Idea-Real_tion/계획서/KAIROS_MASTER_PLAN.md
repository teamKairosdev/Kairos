...
2. [비전 선언문 — 이건 AI 자소서가 아니다](#2-비전-선언문)
   - [사회적 미션 — 사람을 살리고 경력을 살린다](#사회적-미션--사람을-살리고-경력을-살린다)
...
> **"개인의 대시보드"라는 표현은 사용하지 않는다** — 추상적이고 차별성이 없다. 위의 문구를 사용할 것.
...
### 사회적 미션 — 사람을 살리고 경력을 살린다

대한민국에는 **"쉬었음 청년"** 이라는 통계적 표현이 있다. 취업도, 교육도, 훈련도 하지 않고 사실상 방 안에 머무는 청년들. 2026년 기준 약 48만 명으로 추산된다.

이들은 게으른 것이 아니다. **진입 장벽이 너무 높아 출발선에 서지도 못한 것이다.**

- 자기소개서를 쓰려고 하면, 무슨 경험이 있었는지 기억조차 나지 않는다
- AI 도구는 있지만, 어디서부터 시작해야 할지 모른다
- 주변은 모두 준비된 사람들 같아서 더 위축된다
- 결국 "나는 아직 준비가 안 됐어"라는 생각에 방문을 닫는다

**Kairos는 이 문제를 정면으로 해결한다.**

| 장벽 | Kairos의 접근 |
|------|--------------|
| "무슨 경험이 있는지 모르겠어요" | AI와의 대화형 인터뷰로 경험을 발굴 → 지식베이스 자동 구축 |
| "자소서 첫 줄을 쓰는 게 두려워요" | 빈 칸에서 시작하지 않는다. AI가 초안을 만들고 사용자가 다듬는다 |
| "면접 연습을 혼자 할 자신이 없어요" | AI 면접관이 24시간 대기. 평가는 혼자만 볼 수 있다 |
| "내 수준이 어느 정도인지 모르겠어요" | ATS 매칭 점수, 이력서 평가 점수로 객관적 위치 확인 |
| "경제적 여유가 없어요" | 핵심 기능은 무료. 프리미엄은 저렴한 월 구독제 |

> **목표**: "쉬었음 청년"을 **"지금 막 준비를 시작한 청년"** 으로 바꾼다.  
> Kairos는 도구일 뿐만 아니라, **방 안에 있는 사람을 밖으로 나오게 하는 첫걸음**이다.
...
| 플랫폼 | 기술 스택 | 상태 | 비고 |
|--------|-----------|------|------|
| **웹 (SPA)** | Nuxt.js 4 + Vercel | [v] 운영 가능 | 메인 채널 |
| **랜딩/SEO** | Astro 7 (Islands) | [v] 운영 가능 | SEO, 공유 페이지 ISR |
| **모바일 앱** | packages/mobile-bridge (Expo RN) | (계획) 예정 | 네이티브 브릿지 스캐폴딩 완료 |
| **데스크탑 프로그램** | Tauri (Rust + Webview) | (계획) 예정 | 로컬-first 기능 (Electron → Tauri로 변경) |
| **브라우저 확장** | Chrome Extension API | (검토) 고려 | 채용공고 자동 스크래핑 |
