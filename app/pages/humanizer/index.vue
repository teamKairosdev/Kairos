<template>
  <div class="space-y-8">
    <div>
      <h1 class="text-xl font-semibold text-fg-neutral">휴머나이저</h1>
      <p class="text-xs text-fg-neutral-muted mt-0.5">AI 문체를 자연스러운 인간 어조로 변환합니다</p>
    </div>

    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div class="rounded-xl border border-stroke-neutral-muted p-6 bg-neutral-muted space-y-4">
        <h3 class="text-sm font-medium text-fg-neutral">원본 문장</h3>
      ...
      <div class="rounded-xl border border-stroke-neutral-muted p-6 bg-neutral-muted space-y-4">
        <h3 class="text-sm font-medium text-fg-neutral">변환 결과</h3>
        <div v-if="result" class="space-y-4">
          <div class="flex items-center justify-between">
            <span class="text-xs text-fg-neutral-muted">자연스러움</span>
            <span class="text-base font-medium text-fg-neutral">{{ result.styleScore }}</span>
          </div>
          <div class="p-4 rounded-lg bg-neutral-muted text-sm text-fg-neutral leading-relaxed whitespace-pre-wrap">{{ result.humanizedText }}</div>
          <div class="p-4 rounded-lg bg-neutral-muted text-xs space-y-2">
            <div class="font-medium text-fg-neutral-muted">변환 요약</div>
            <p class="text-fg-neutral-muted">{{ result.changesSummary }}</p>
        ...
        <div v-else class="h-64 flex items-center justify-center text-center text-fg-neutral-muted">
